# LE Mesh Sensor Temperature application

## Overview
This demo application shows an implementation of a LE Mesh temperature sensor. The app is based on the LE Mesh Sensor Server model.
...
- Temperature measurement using the on board thermistor on the CYBT-213043-MESH/CYBLE-343072-MESH/CYW920819EVB-02 Evaluation Kit
- Usage of LE Mesh Sensor Server model

## Instructions
To demonstrate the app, work through the following steps:

1. Build and download the application to the CYBT-213043-MESH/CYBLE-343072-MESH/CYW920819EVB-02 Evaluation Kit
2. Use the Android MeshController or Windows Mesh Client app and provision the temperature sensor
3. After successful provisioning, use the Android MeshController/ Windows Mesh Client app to configure the below parameters of the sensor:<br/>
    a) configure sensor to publish the sensor data to a group(all-nodes, all-relays)<br/>
    b) configure publish period : publish period defines how often the user wants the sensor to publish the data. For testing, you can set it to 5000 msec<br/>
    c) set cadence of the sensor (optional step):<br/>
        * set minimum interval in which sensor data has to be published.<br/>
        * set the range in which the fast cadence has to be observed.<br/>
        * set the fast cadence period (how fast the data has to be published with respect to publish period).<br/>
...
5. To change the temperature on the thermistor, you can keep your finger on the onboard sensor and see the changes.
